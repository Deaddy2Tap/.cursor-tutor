...
menuloop = [pygame.image.load(f'menu{i}.png').convert_alpha() for i in range(1, 11)]
...
five_seven_enemy = EnemyFiveSeven(random.randint(0, 9), FSENEMY_RADIUS)
ak_enemy = EnemyAK(random.randint(0, 9), AKENEMY_RADIUS)
buckshot_enemy = EnemyBuckshot(random.randint(0, 9), BSENEMY_RADIUS)
...
def redraw():
    ...
    if five_seven_enemy.is_alive:
        screen.blit(five_idle, (five_seven_enemy.x, five_seven_enemy.y))
        
    if ak_enemy.is_alive:
        screen.blit(ak_idle, (ak_enemy.x, ak_enemy.y))
    
    if buckshot_enemy.is_alive:
        screen.blit(buckshot_idle, (buckshot_enemy.x, buckshot_enemy.y))
